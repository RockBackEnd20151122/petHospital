...
return {
	...
	*/
	...
	/**
	* 批量设置cookie
	* @param {Json} options json对象 --必填
	* @example
	* $.cookie.setOptions(options.key,options.value,options.domain,options.path,options.time); //设置cookie
	*/
	setOptions:function(options){
		for(var key in options){
			this.set(key,options[key],options.domain,options.path,options.time);
		}
	},
	/**
	...
	*/
	clear:function(domain){
		...
		// 删除所有cookie
		for (var i in rs){
			document.cookie = rs[i] + "=;expires=Mon, 26 Jul 1997 05:00:00 GMT;domain="+domain+"; path=/; " ;
		}
	}
