...
function fmoney(s, n)//将数字转换成逗号分隔的样式,保留两位小数s:value,n:小数位数   
{
	...
	var l = s.split(".")[0].split("").reverse(), i,
	r = s.split(".")[1];
	t = "";
...
(function($){
    ...
    function getDynData(path, method, cb){
		...
		}); // for now, do not care about fail cases;
    }
        ...
        if(!$(that).attr("clickValid")){
            ...
            setTimeout(function(){
                $.ajax({url:"/captcha",
                    ...
                    var $t = $(that),
                        idEl = $('#'+$t.data('id').replace(/[.]/g, "\\.")),
                        imgEl = $t.parent().find('.captcha');
	...
	$('.otpSend').each(function(){
		...
		// TODO add in to verify if mobile is valid
			...
			if ( $t.prop('disabled') !== undefined && counter <= 0 ) {
				...
				}).done(function(){ // while done ,then count down
					// sendOTP
					var countdownFn = sendOTPCountdown($t);
					$t.prop('disabled', 'disabled');
					$t.data('sendOTPCount', 61)
					  .data('sendOTPtimeout', setInterval(countdownFn, 1000));
					countdownFn();
				}); // this is too easy to break right now...
	...
	$('.textarea-count').each(function(){
		...
		function modify(){
			var max = $ta.prop('maxlength') || 50,
				count = $ta.val().length; // TODO consider should make this unchangable
			if(count >= max){
	...
	function validateIdentCard(el) {
		var $t = $(el), 
			val = $t.val().trim(),
			$type = $(($t.data('type') || "").replace(/[.]/g, "\\.")),
			type = $type.val() || "CHINA_IDENTIFICATION_CARD",
			errors = [];
		...
		if ( "CHINA_IDENTIFICATION_CARD" === type ) {
        	$('.password-ruled')
	    		.data('bdYear', undefined)
	    		.data('bdMonth', undefined)
	    		.data('bdDate', undefined);
...
//			            return errors;
	...
	var rules = [
	     [/(\d)\1\1\1\1\1/, "不可重复同一数字6次以上"],
	     [/(\d\d)\1\1/, "不可重复2个数字组合3次以上"],
	     [/(\d\d\d)\1/, "不可重复3个数字组合"]
	];
//	var charGroups = [/[A-Z]/, /[a-z]/,/[0-9]/, /[@#$&%\^\*\(\)\[\]\!]/];
	var charGroups = [/[A-Za-z]/,/[0-9]/, /[@#$&%\^\*\(\)\[\]\!]/], rNum = /[0-9]/,
		rPNotAllowed = /[^A-Za-z0-9@#$&%\^\*\(\)\[\]\!]/;
	function isNum(i) {
		return i && rNum.test(i);
	}
	
	 function isAllFill(id){//判断是否所有带有required属性的input都已经填充。
		    var  mark = true;
		    if(userAgent()== "msie"){    	
	 ...
	 function loginPayIsSame(id){//判断支付密码和登录密码是否一致
		    if(id != "signupForm"){
		    	return true;
		    }
	...
	function displayErrors(el, evenEmpty, focus) {
		var $t = $(el),
			...
			errorBlock = $t.parent().parent().children('.help-block.error');
		...
		if ( errors || evenEmpty ) {
 			...
 			if ( count > 0 || evenEmpty ) {
 				...
 				if ( count > 0 ) {
 					...
 					if ( errorBlock.is(":hidden")) { errorBlock.slideDown(100); }
 				}
 			}
 		} else {
		...
		if ( !namespace ) {
			...
			for ( var name in errors ) {
				if ( name.indexOf(namespace) === 0 ) {
					$t.parents('.form-group').find('.error-container[data-error-name="'+name+'"]');
					delete errors[name]; // very naive implementation, think of a better one in the future
				} else { ++errorCount; }
			}
			$t.data('errors', errors);
		}
...
(function($){
	...
	function emptyErrors(el) {
		var $el = $(el),
            errorBlock = $(el).parent().parent().children('.help-block.error'),
            $fromGroup = $el.closest('.form-group');
		errorBlock.children().remove();
	...
	function renderErrors(el, validationContext) {
		var $t = $(el),
			errors = $(el).data('validationContext') || {},
			errorBlock = $t.parent().parent().children('.help-block.error');
		...
		if ( errors ) {
			...
			for ( var name in errors) {
				++count;
				if ( name in renderers) {renderers[name](el, name, validationContext.errors[name]).appendTo(errorBlock);}
				else {basicRender(el, name, validationContext.errors[name]).appendTo(errorBlock);}
			}
			if ( count > 0 ) {
	...
	var ValidationContext = function(el){this.errors={};this.hasErrors=false;this.el = $(el);};
	...
	$("body").on('change blur keyup', 'textarea, input ', function(e){
		var $t = $(this);
	 	...
	 	$t.data("validationTimeout", setTimeout(function(){
	 		...
	 		if (  $t.data("validationOldValue") === $t.val() ) { return; }
	 		emptyErrors($t);
	...
	$('form').on('click', 'input[type=submit], input[type=button], button', function(e){
		if ($(this).closest('form').hasClass('submitting')) { return e.preventDefault(); }
	});
	
    $('form:not(".simpleForm")').on('submit', function (e) {
        var $form = $(this), hasErrors = false;
        if ($form.hasClass("submitting")) { return e.preventDefault(); }
...
(function($) {
	...
	Loading.prototype = {
		init : function() {
			var sHtm = '<div class="loading">';
			this.loading = $(sHtm).appendTo(this.$box.css('position','relative').append('</div><div class="mask"></div>')).css({
				width:32,
				height:32,
				position:'absolute',
				top:'50%',
